	...
	public static void load(JavaPlugin plugin) {
		...
		if (!dir.exists()) {
			...
			try {
				...
				plugin.getServer().getConsoleSender().sendMessage("[NeoPayments] Não foi possível criar um arquivo de exemplo.");
			}
			return;
		}
		...
		for (File f : files) {
			...
			if (category == null) {
				plugin.getServer().getConsoleSender().sendMessage("[NeoPayments] Não foi possível encontrar a categoria " + config.getString("category") + " para o produto " + config.getString("name") + ".");
				continue;
			}
			...
			if (material == null) {
				material = BARRIER;
				plugin.getServer().getConsoleSender().sendMessage("[NeoPayments] Não foi possível encontrar o material " + config.getString("icon.material") + " para o produto " + product.getName() + ". Portanto foi alterado para BARRIER.");
			}
			...
			for (String rawGateway : config.getStringList("gateways")) {
				...
				if (gatewayType == null) {
					plugin.getServer().getConsoleSender().sendMessage("[NeoPayments] Não foi possível encontrar a gateway " + rawGateway + " para o produto " + product.getName() + ".");
					continue;
				}
			...
			for (String rawAction : config.getConfigurationSection("actions").getKeys(false)) {
				...
				if (type == null) {
					plugin.getServer().getConsoleSender().sendMessage("[NeoPayments] Não foi possível encontrar a ação " + rawAction + " para o produto " + product.getName() + ".");
					continue;
				}
				...
				try {
					...
					Bukkit.getConsoleSender().sendMessage("[NeoPayments] Não foi possível encontrar o som " + config.getString("actions." + rawAction + ".sound") + " para o produto " + product.getName() + ". Portanto foi alterado para LEVEL_UP.");
				}
			...
			Bukkit.getConsoleSender().sendMessage("[NeoPayments] O produto " + product.getName() + " foi carregado com sucesso.");
		}
	}
}
